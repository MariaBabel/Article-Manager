...
namespace article_manager.Services
{
    ...
    public class ArticleCategoriesService : ICRUDService<ArticleCategoryListItem, ArticleCategoryItem>
    {
        ...
        [Inject] TokenProvider TokenProvider { get; set; }
        ...
        public async Task<ArticleCategoryListItem[]> GetList()
        {

            // _httpClient.DefaultRequestHeaders.Authorization =
            //     new AuthenticationHeaderValue("Bearer", TokenProvider.AccessToken);

            var response = await _httpClient.GetJsonAsync<List<ArticleCategory>>($"api/articleCategory");

            return response.Select(catergory => new ArticleCategoryListItem()
            {
                ID = catergory.Id,
                Name = catergory.Name
            }).ToArray();
